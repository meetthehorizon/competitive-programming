...
Realise that the ans is argmin max (ti + abs(xi - x)) 
also see that we can replace (xi, ti) by (xi-ti) && (xi+ti)
now if the final array belongs to (min, max) the answer will be the average of them.
any other position will produce a larger time


Implementation:
not that (min+max)/2.0 gives wrong answer due to accuracy 
better seperately print .5 if min+max is odd
*/
...
void solve()
{
    ...
    ll n;
    cin>>n;
    vll vec = input_vec(n); vll t = input_vec(n);
    ll min = vec[0] - t[0];
    ll max = vec[0] + t[0];
    FOR(i, n){
        min = minll(min, vec[i] - t[i]);
        max = maxll(max, vec[i] + t[i]);
    }
    cout<<(min+max)/2;
    if((min+max)%2) cout<<".5"<<'\n';
    else cout<<'\n';
