...
void solve()
{
    ...
    ll n;
    cin>>n;
    vector<vll> vec(3, vll(n, 0));
    FOR(i, n) cin>>vec[0][i]>>vec[1][i]>>vec[2][i];        
    FOR(i, n) if(i) FOR(j, 3) vec[j][i] += maxll(vec[(j+1)%3][i-1], vec[(j+2)%3][i-1]);
    cout<<maxll(vec[1].back(), maxll(vec[0].back(), vec[2].back()));
}
